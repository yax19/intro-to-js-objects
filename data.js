...
const pokemon = [
  ...
  const pokemon2 = [
    ...
    { name: "Pikachu", type: "Electric", HP: 35, starter: false },
    { name: "Jigglypuff", type: "Normal", HP: 115, starter: false },
    { name: "Onix", type: "Rock", HP: 35, starter: false }
  ];
  ...
  if (starterPokemon) game.party.push(starterPokemon);
  
  // Select three more Pokémon based on attributes (e.g., highest HP)
  const additionalPokemon = pokemon
    .filter(p => !p.starter) // Exclude starters
    .sort((a, b) => b.HP - a.HP) // Sort by HP descending
    .slice(0, 3); // Select top 3
  
  // Add selected Pokémon to party
  ...
  // Select a starter Pokémon
  const starterPokemon2 = pokemon.find(p => p.starter);
  if (starterPokemon) game.party.push(starterPokemon);
  
  // Select three more Pokémon based on attributes (e.g., highest HP)
  const additionalPokemon2 = pokemon
    .filter(p => !p.starter) // Exclude starters
    .sort((a, b) => b.hp - a.hp) // Sort by HP descending
    .slice(0, 3); // Select top 3
  ...
  const pokemon4 = [
    { id: 1, name: "Bulbasaur", evolvesTo: { id: 2, name: "Ivysaur" }, starter: true },
    { id: 4, name: "Charmander", evolvesTo: { id: 5, name: "Charmeleon" }, starter: true },
    { id: 7, name: "Squirtle", evolvesTo: { id: 8, name: "Wartortle" }, starter: true },
    { id: 25, name: "Pikachu", evolvesTo: { id: 26, name: "Raichu" }, starter: false }
  ];
  ...
  // Select a starter Pokémon and add it to the party
  const starter = pokemon4.find(p => p.starter);
  if (starter) {
    game3.party.push(starter);
  }
  
  // Evolve the starter Pokémon
  const starterIndex = game3.party.findIndex(p => p.starter);
  if (starterIndex !== -1) {
    const evolvedForm = {
      id: starter.evolvesTo.id,
      name: starter.evolvesTo.name,
      starter: false // Once evolved, it's no longer a starter
    };
    game3.party.splice(starterIndex, 1, evolvedForm); // Replace the old Pokémon with its evolved form
  }
...
// EXCERCISE 8
  ...
  const ex9 = [
    ...
    { id: 25, name: "Pikachu", starter: false },
    { id: 39, name: "Jigglypuff", starter: false }
  ];
  
  // Find all starter Pokémon
  const starters = ex9.filter(p => p.starter);
  
  // Print their names using forEach
  starters.forEach(p => console.log(p.name));//
  

  //EXCERCISE 11
  // Assuming game object already exists
const mod = {
  party: [],
  items: [
...
// Adding the modified catchPokemon method
mod.catchPokemon = function (pokemonObj) {
  // Find the pokeball item in game.items
  const pokeball = this.items.find(item => item.name === "pokeball");

  // Decrease the quantity of pokeballs
  if (pokeball) {
    pokeball.quantity--;
  }
...
// Calling the method to catch Bulbasaur
mod.catchPokemon(bulbasaur);

console.log(mod.party); // Should contain Bulbasaur
console.log(mod.items); // Should show pokeball quantity decreased by 1
